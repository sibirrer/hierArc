...
class TestLightProfile:
    def setup_method(self):
        self.kw_sersic = {"R_sersic": 1.5, "amp": 1.0, "n_sersic": 2.5,
                          'e1': 0.1, 'e2': 0.01, "center_x": 0.2,  "center_y": -0.1,}
    ...
    def test_mge_lum(self):
        ...
        surf_lum, sigma_lum  = lp.mge_lum_tracer(r_mge, [self.kw_hernquist],
                                                 n_gauss=20, linear_solver=True)
        mge_surf_1d = self._mge(r_test, surf_lum, sigma_lum)
        hernq = Hernquist()
        hernq_surf_1d = hernq.function(
            x=r_test, y=0,
            Rs=self.kw_hernquist["Rs"], amp=self.kw_hernquist["amp"]
        )
    ...
    def test_mge_lum_mge_prof(self):
        ...
        surf_lum, sigma_lum = lp.mge_lum_tracer(r_test, [kw_mge],
                                                n_gauss=20, linear_solver=True)
        mge_surf_1d = self._mge(r_test, surf_lum, sigma_lum)
